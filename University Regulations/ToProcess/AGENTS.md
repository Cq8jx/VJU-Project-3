...
4. University Regulations ワークフロー
5. LaTeX / Notes 運用
6. その他テンプレート・環境設定
7. 用語集（人名 / 用語）
...
1. `Guide/Google Meet/` には日本語・英語・ベトナム語版を同時に維持し、章構成（目的→全体の流れ→手順セクション）と用語を三言語で揃える。  
2. 端末操作や共同主催者設定などの手順直後に対応スクリーンショットを必ず挿入し、`Guide/Google Meet/Fig/` 配下へ用途が分かるファイル名（例: `google-meet-add-cohost.png`）で保存する。  
...
### 4.1 作業フロー
1. **下準備**  
   - 対象は `University Regulations/Unprossed/` の MD / PDF ペア。  
2. **ファイル名整理**  
...
| QA | Educational Testing and Quality Assurance Office | 教育試験・質保証課 | Phòng Khảo thí và Đảm bảo chất lượng giáo dục |  |
...
## 8. Git同期の教訓
- たとえ fetch/push に失敗しても、`fatal: bad object refs/Icon?` や `did not send all necessary objects` が出る場合は、`.git/refs` 以下に `Icon\r`（CRLF を含むゴーストファイル）が残っていないか調べる。`find .git -name $'Icon\\r' -delete` で除去すると同期が復旧することがある。
- 同様の問題が再発したら、ローカルの `.git/refs` と `.git/logs` に CRLF付きファイルが入っていないか（`find ... -name $'Icon\\r'`）を確認すること。Git は `?` を含む参照名を invalid ref と見なすため、手動で削除しない限りリモート通信が止まる。
